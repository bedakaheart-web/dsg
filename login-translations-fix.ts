/**
 * FIX for the login page showing raw keys like "login.headline1", "login.formTitle", etc.
 *
 * ROOT CAUSE:
 *   src/pages/Login.tsx calls t("login.headline1"), t("login.formTitle"),
 *   t("login.labels.email"), t("login.errors.unconfirmedEmail"), etc.
 *   But translations.ts only has an `auth` namespace (different key names,
 *   less content) — there is no `login` namespace at all, in ANY language,
 *   including English. Your t() helper falls back to English, and when the
 *   key is missing there too it just returns the raw path string — which is
 *   exactly what you're seeing rendered on screen. It would look identical
 *   in English; Tagalog just isn't hiding the actual bug.
 *
 * HOW TO APPLY:
 *   1. In `TranslationDict` (top of translations.ts), add the `login` type
 *      shown below anywhere inside the type (e.g. right after `auth`).
 *   2. In each of the 8 language objects (en, tl, ceb, ko, zh, ja, ru, ar),
 *      paste the matching `login: { ... }` block right after that
 *      language's `auth: { ... },` block (before `cards: {`).
 */

// ── 1. Add to TranslationDict type ──────────────────────────────
/*
  login: {
    backToHome: string;
    formEyebrow: string;
...
// ── 2. EN — insert after auth: {...}, in the `en` object ────────
/*
    login: {
...
// ── TL — insert after auth: {...}, in the `tl` object ────────────
/*
    login: {
...
// ── CEB — insert after auth: {...}, in the `ceb` object ──────────
/*
    login: {
...
// ── KO — insert after auth: {...}, in the `ko` object ─────────────
/*
    login: {
      backToHome: "홈으로 돌아가기",
      formEyebrow: "보안 접속",
...
// ── ZH — insert after auth: {...}, in the `zh` object ─────────────
/*
    login: {
      backToHome: "返回首页",
      formEyebrow: "安全登录",
...
// ── JA — insert after auth: {...}, in the `ja` object ─────────────
/*
    login: {
      backToHome: "ホームに戻る",
...
// ── RU — insert after auth: {...}, in the `ru` object ─────────────
/*
    login: {
...
// ── AR — insert after auth: {...}, in the `ar` object ─────────────
/*
    login: {

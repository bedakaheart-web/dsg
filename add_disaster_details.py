...
# We need to add disasterDetails and goBagItems before the closing "},"

# Let's find all safetyTips blocks and check if they have disasterDetails
blocks = re.split(r'(safetyTips:\s*\{)', content)
# This won't work well. Let me try a different approach.

# Find each safetyTips block that doesn't have disasterDetails
...
# After "categories: { ... }," there's a newline and "    },"
# We need to insert disasterDetails and goBagItems before that "    },"

# Let's find all occurrences of the pattern:
# categories: { ... },\n    },
# and check if they're inside a safetyTips block that's missing disasterDetails

# Simpler approach: find all "categories: { ... }," lines and check if the next line is "    },"
# and if the safetyTips block doesn't have disasterDetails
...
# Actually, let me just find all safetyTips blocks and fix them one by one

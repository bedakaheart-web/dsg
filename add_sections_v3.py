...
# Sections to add to each language data block after report: { ... },
# and before signup: { ... }
...
# Strategy: find each `    report: {` at 4-space indent (top level in language block)
# Then find its closing `    },` and check if dashboard follows

# First, find all positions of `    report: {` that are followed by `heroTitle:` with a string value
# This distinguishes data blocks from type definitions

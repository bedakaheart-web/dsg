...
.rd-logo-ring { position: relative; width: 40px; height: 40px; flex-shrink: 0; }
...
.rd-main { margin-left: 260px; flex: 1; display: flex; flex-direction: column; position: relative; z-index: 1; height: 100%; min-width: 0; overflow: hidden; }
...
.rd-stat.sv-red .rd-stat-num, .rd-stat.sv-red .rd-stat-icon, .rd-stat.sv-red .rd-stat-tag { color: var(--danger); }
...
export default function RespondersDashboard() {
  ...
  // ── ✅ FIX: Set responder status to on_duty on login ──────────────────────
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (user) {
          ...
          // ✅ Mark this responder as on_duty in the database
          await supabase
            .from("profiles")
  ...
  // ── ✅ FIX: Set responder status to off_duty on logout ────────────────────
  const handleLogout = async () => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        // ✅ Mark this responder as off_duty in the database
        await supabase
          .from("profiles")

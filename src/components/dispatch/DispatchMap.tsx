...
export function incidentPin(color: string, selected = false): L.DivIcon {
  ...
  return L.divIcon({
    className: "",
    html: `<div style="width:${size}px;height:${size}px;border-radius:9999px;background:${color};display:flex;align-items:center;justify-content:center;font-size:${selected ? 18 : 14}px;border:${selected ? 3 : 2}px solid #0d1117;box-shadow:0 2px 12px rgba(0,0,0,.5);">📍</div>`,
    iconSize: [size, size],
...
export function unitPin(heading: number | null): L.DivIcon {
  ...
  return L.divIcon({
    className: "",
    html: `<div style="width:38px;height:38px;border-radius:9999px;background:#0066FF;display:flex;align-items:center;justify-content:center;font-size:18px;border:3px solid #fff;box-shadow:0 2px 16px rgba(0,0,0,.5),0 0 0 4px rgba(0,102,255,.35);"><span style="display:inline-block;${rot ? `transform:${rot};` : ""}">🚑</span></div>`,
    iconSize: [38, 38],
    iconAnchor: [19, 19],
  });
}
  ...
  return (
    <>
      <style>{`
        ...
        .dm-hud { position: absolute; top: 12px; left: 12px; right: 12px; z-index: 600; display: flex; justify-content: flex-start; pointer-events: none; }

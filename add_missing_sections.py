...
# Define the sections to add (dashboard, history, alerts, reportDetail)
# These need to be added after the "report" block closes and before "signup" opens
# in each language block that's missing them

sections_template = """    dashboard: {
...
# Find all language blocks that are missing dashboard/history/alerts/reportDetail
# Pattern: after "report: { ... }," and before "signup:" or before the closing "}," of the language block

# Find each language block
language_blocks = re.split(r'(\b(?:en|tl|ceb|ko|zh|ja|ru|ar):\s*\{)', content)
# This won't work well. Let me try a different approach.

# Find all positions of "signup:" that are inside language blocks
# and check if the preceding "report:" block is followed by "dashboard:"

# Actually, let me find each language block's "report" closing and check what follows
# Pattern: after "success: { ... }," (the last part of report) and before "signup:" or "},"

# Let me find all occurrences of "report:" blocks and check what comes after
...
for m in matches:
    ...
    # Look at what comes after this block
    after_block = content[block_end:block_end+100]
    if 'dashboard:' not in after_block and 'signup:' not in after_block:
        # This block is missing dashboard/history/alerts/reportDetail
        # Insert them before the closing "},"
        closing_pos = block.rfind('},')
        if closing_pos > 0:
            content = content[:idx + closing_pos] + '\n' + sections_template + content[idx + closing_pos:]

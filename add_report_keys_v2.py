...
# and ends with `  },` at 2-space indent

# Find all language block starts
lang_pattern = re.compile(r'(    (en|tl|ceb|ko|zh|ja|ru|ar): \{)')
lang_matches = list(lang_pattern.finditer(content))

print(f"Found {len(lang_matches)} language blocks")

# For each language block, find the report section and add missing keys
for lang_match in lang_matches:
    lang_code = lang_match.group(2)
    block_start = lang_match.start()
    
    # Find the end of this language block (next `  },` at 2-space indent, or end of file)
    # We need to find the matching closing brace
    # Simple approach: find the next `  },` that is at 2-space indent and not inside a nested block
    
    # Find the report section within this block
    report_match = re.search(r'(    report: \{)', content[block_start:])
    if not report_match:
        continue
    
    report_start = block_start + report_match.start()
    
    # Find the closing `    },` for the report block
    ...
    if closing_match:
      ...
      form: {
        ...
        print(f"  Added keys to {lang_code} block at position {insert_pos}")
